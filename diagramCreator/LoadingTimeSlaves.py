...
plt.legend((p8[0], p7[0], p6[0], p5[0], p3[0], p2[0], p1[0]), ('local index creation', 'transfer to slaves', 'join resp. adjustment', 'statistics collection', 'final dictionary encoding', 'cover creation', 'initial dictionary encoding'), bbox_to_anchor=(0, 1.01, 0.4, .102), loc=3, ncol=1, mode="expand", borderaxespad=0.)
plt.axis('tight')
if imageType=="latex":
  fig.tight_layout(rect=(-0.02,-0.02,1.025,0.83))

if imageType=="latex":
  latex.savefig(outputDir,'loadingTimesSlaves')
else:
  plt.savefig(outputDir+'/loadingTimesSlaves.'+imageType, bbox_inches='tight')
