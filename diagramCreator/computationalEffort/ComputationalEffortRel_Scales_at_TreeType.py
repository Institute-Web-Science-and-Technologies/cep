...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    treeTypes[treeType][scale][cover][query] = { "Computational Effort":long(row[9]), "Entropy":float(row[10]), "Workload Imbalance":float(row[12])}
  ...
  for treeType in treeTypes.keys():
    for baseScale in treeTypes[treeType].keys():
        ...
        for j, cover in enumerate(coverSet):
          ...
          for query in queryGroups:
            ...
            if baseDataTransfer > 0:
              dataRows[scale][cover].append((currentDataTransfer-baseDataTransfer)/float(baseDataTransfer)*100);
      ...
      if imageType=="latex":
        ...
        latex.savefig(outputDir,'computationalEffort'+'_relativeTo_'+baseScale+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_sortedByCover')
      else:
        plt.legend(bbox_to_anchor=(-0.2, 1.04, 1.2, .102), loc=3, ncol=2, mode="expand", borderaxespad=0.)
        plt.savefig(outputDir+'/computationalEffort'+'_relativeTo_'+baseScale+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_sortedByCover.'+imageType, bbox_inches='tight')
      plt.close('all')
