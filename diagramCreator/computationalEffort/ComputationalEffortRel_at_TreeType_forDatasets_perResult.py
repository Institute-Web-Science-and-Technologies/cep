  ...
  for treeType in treeTypes.keys():
    for baseCover in treeTypes[treeType].keys():
      for baseDataset in treeTypes[treeType][baseCover]['20'].keys():
        ...
        for i, cover in enumerate(coverSet):
          ...
          for j, dataset in enumerate(datasetSet):
            ...
            for query in queryGroups:
              baseDataTransfer = treeTypes[treeType][baseCover]['20'][baseDataset][query][measurementType]/float(resultSetSize[query][prettyPrint(baseDataset)])
              currentDataTransfer = treeTypes[treeType][cover]['20'][dataset][query][measurementType]/float(resultSetSize[query][prettyPrint(dataset)])
        ...
        if imageType=="latex":
          latex.latexify(fig_height=3.5,fig_width=7,scale=1)
        ...
        for i, dataset in enumerate(datasetSet):
          for j, cover in enumerate(coverSet):
            ...
            c+=1
        plt.xlabel("Queries")
        plt.ylabel("\\parbox{200pt}{\centering " + measurementType+'\\\\(log scale, change to '+baseCover + '\\\\with ' + prettyPrint(baseDataset) + ' triples' +' in \\%)'+"}")
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'computationalEffort'+'_relativeTo_'+baseCover+prettyPrint(baseDataset)+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_showingDatatasets'+"_perQueryResult")
        else:
          plt.legend(bbox_to_anchor=(-0.4, 1.04, 1.4, .102), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/computationalEffort'+'_relativeTo_'+baseCover+prettyPrint(baseDataset)+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_showingDatataset'+"_perQueryResult"+'.'+imageType, bbox_inches='tight')
        plt.close('all')
