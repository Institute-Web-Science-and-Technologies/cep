...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    selectivitys[selectivity][cover][query] = { "Total":long(row[8]), "Entropy":float(row[9]), "Standard Deviation":float(row[10])}
...
for measurementType in ["Total", "Entropy", "Standard Deviation"]:
  for selectivity in selectivitys.keys():
    ...
    plt.title('Computational effort for selectivity ' + selectivity)
    ...
    plt.legend()
    plt.savefig(outputDir+'/computationalEffort_'+measurementType+'_selectivity-'+selectivity+'_forAll_covers.'+imageType, bbox_inches='tight')
