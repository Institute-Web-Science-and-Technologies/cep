      ...
      for baseCover in treeTypes[treeType][baseScale].keys():
        ...
        for i, scale in enumerate(scaleSet):
          colorValue = colors[i]#colors[c]
            ...
            #print treeType+"-"+baseCover+"-"+scale+": "+str(change)+" average: "+str(sum(change)/len(change))+" median: "+str((sorted(change)[5]+sorted(change)[6])/2.)
        ...
        plt.text(2,0.6,'finished')
        plt.text(8.5,0.6,'aborted')
        #plt.axis('tight')
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'computationalEffort'+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover)
        else:
          plt.legend(bbox_to_anchor=(0, 1.04, 1, .103), loc=3, ncol=1, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/computationalEffort'+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover+'.'+imageType, bbox_inches='tight')
        plt.close('all')
