  ...
  for treeType in treeTypes.keys():
    for baseCover in treeTypes[treeType].keys():
      for baseScale in treeTypes[treeType][baseCover].keys():
        ...
        plt.text(2,0.7 if baseScale == 500000000 else 0.45 if baseScale==1000000000 else 0.8,'finished')
        plt.text(8.5,0.7 if baseScale == 500000000 else 0.45 if baseScale==1000000000 else 0.8,'aborted')
        #plt.axis('tight')
        #plt.title(measurementType + ' for ' + treeType + ' trees sorted by number of chunks' + " relative to " + baseCover,y=1.25)
        if imageType=="latex":
          plt.legend(bbox_to_anchor=(0, 1.01, 1, .102), loc=3, ncol=1, mode="expand", borderaxespad=0., borderpad=0.2, labelspacing=0.2)
          fig.tight_layout(rect=(-0.04,-0.045,1.04,0.91))
          latex.savefig(outputDir,'computationalEffort'+'_'+measurementType+'_treeType-'+treeType+'_dataset-'+prettyPrint(baseScale) + '_forDatasets')
        else:
          plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.4, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/computationalEffort'+'_'+measurementType+'_treeType-'+treeType+'_dataset-'+prettyPrint(baseScale) + '_forDatasets'+'.'+imageType, bbox_inches='tight')
        plt.close('all')
