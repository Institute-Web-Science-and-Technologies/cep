...
for measurementType in ["\\# Cut Edges"]:
    for baseCover in treeTypes.keys():
      ...
      if imageType=="latex":
        ...
        latex.savefig(outputDir,'cutEdges'+'_relativeTo_'+baseCover+'_comparingCoversForScales')
      else:
        plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.4, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
        plt.savefig(outputDir+'/'+'cutEdges'+'_relativeTo_'+baseCover+'_comparingCoversForScales'+'.'+imageType, bbox_inches='tight')
      plt.close('all')
