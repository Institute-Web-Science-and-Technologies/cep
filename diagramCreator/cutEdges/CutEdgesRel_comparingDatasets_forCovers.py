...
for measurementType in ["\\# Cut Edges"]:
    for baseCover in treeTypes.keys():
      ...
      c=0
      for i, cover in enumerate(coverSet):
        colorValue = "red" if cover == 500000000 else "green" if cover == 1000000000 else "blue"#colors[c]
        rects.append(plt.bar(index + i*1*bar_width+0.5*bar_width, np.array(dataRows[cover]), bar_width, color=colorValue, label=prettyPrint(cover) + " triples", linewidth=0.5))
        c+=1
      #plt.xlabel("Number of Chunks")
      plt.ylabel("\\parbox{200pt}{\centering " + measurementType+'\\\\(change to '+prettyPrint(baseCover)+ ' triples in \\%)'+"}")
      ax.yaxis.set_label_coords(-0.08, 0.45)
      plt.xticks(index + 0.5, np.array(sorted(treeTypes[baseCover].keys())))
      ...
      if imageType=="latex":
        ...
        fig.tight_layout(rect=(-0.03,-0.05,1.03,0.99))
        latex.savefig(outputDir,'cutEdges'+'_relativeTo_'+prettyPrint(baseCover)+'_comparingDatasetsForCovers')
      else:
        plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.4, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
        plt.savefig(outputDir+'/'+'cutEdges'+'_relativeTo_'+prettyPrint(baseCover)+'_comparingDatasetsForCovers'+'.'+imageType, bbox_inches='tight')
      plt.close('all')
