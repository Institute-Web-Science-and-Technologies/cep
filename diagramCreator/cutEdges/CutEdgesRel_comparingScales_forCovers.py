...
for measurementType in ["\\# Cut Edges"]:
    for baseCover in treeTypes.keys():
      ...
      c=0
      for i, cover in enumerate(coverSet):
        colorValue = "red" if cover == "10" else "green" if cover == "20" else "blue"#colors[c]
        rects.append(plt.bar(index + i*1*bar_width+0.5*bar_width, np.array(dataRows[cover]), bar_width, color=colorValue, label=cover + " chunks", linewidth=0.5))
        c+=1
      #plt.xlabel("Number of Chunks")
      plt.ylabel("\\parbox{200pt}{\centering " + measurementType+'\\\\(change to '+baseCover+ ' chunks in \\%)'+"}")
      ...
      if imageType=="latex":
        ...
        latex.savefig(outputDir,'cutEdges'+'_relativeTo_'+baseCover+'_comparingScalesForCovers')
      else:
        plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.4, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
        plt.savefig(outputDir+'/'+'cutEdges'+'_relativeTo_'+baseCover+'_comparingScalesForCovers'+'.'+imageType, bbox_inches='tight')
      plt.close('all')
