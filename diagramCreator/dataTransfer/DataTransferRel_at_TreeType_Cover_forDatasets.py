    ...
    for baseScale in treeTypes[treeType].keys():
      for baseCover in treeTypes[treeType][baseScale].keys():
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'dataTransfer'+'_relativeTo_'+prettyPrint(baseScale)+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover)
        else:
          plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.4, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/DataTransfer'+'_relativeTo_'+prettyPrint(baseScale)+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover+'.'+imageType, bbox_inches='tight')
        plt.close('all')
