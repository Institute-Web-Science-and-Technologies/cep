...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    dataSources[dataSource][cover][query] = { "Data Transfer":long(row[8])}
...
for measurementType in ["Data Transfer"]:
  for dataSource in dataSources.keys():
    ...
    plt.title('Data transfer for ' + dataSource + ' data source' + ( "" if int(dataSource) == 1 else "s"))
    ...
    plt.legend()
    plt.savefig(outputDir+'/dataTransfer_'+measurementType+'_dataSource-'+dataSource+'_forAll_covers.'+imageType, bbox_inches='tight')
