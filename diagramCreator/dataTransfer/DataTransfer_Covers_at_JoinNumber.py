...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    joinNumbers[joinNumber][cover][query] = { "Data Transfer":long(row[8])}
...
for measurementType in ["Data Transfer"]:
  for joinNumber in joinNumbers.keys():
    ...
    plt.title('Data transfer for ' + joinNumber + ' join' + ( "" if int(joinNumber) == 1 else "s"))
    ...
    plt.legend()
    plt.savefig(outputDir+'/dataTransfer_'+measurementType+'_joinNumber-'+joinNumber+'_forAll_covers.'+imageType, bbox_inches='tight')
