...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    joinTypes[joinType][cover][query] = { "Data Transfer":long(row[8])}
...
for measurementType in ["Data Transfer"]:
  for joinType in joinTypes.keys():
    ...
    plt.title('Data transfer for join type ' + joinType)
    plt.xticks(index + 0.5, np.array(queryGroups))
    plt.setp(plt.gca().get_xticklabels(), rotation=45, horizontalalignment='right')
    plt.axis('tight')
    plt.legend()
    plt.savefig(outputDir+'/dataTransfer_'+measurementType+'_joinType-'+joinType+'_forAll_covers.'+imageType, bbox_inches='tight')
