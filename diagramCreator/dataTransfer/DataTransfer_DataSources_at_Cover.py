...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    coverTypes[cover][dataSource][query] = { "Data Transfer":long(row[8])}
...
for measurementType in ["Data Transfer"]:
  for cover in coverTypes.keys():
    ...
    plt.axis('tight')
    plt.legend()
    plt.savefig(outputDir+'/dataTransfer_'+measurementType+'_cover-'+cover+'_forAll_dataSources.'+imageType, bbox_inches='tight')
