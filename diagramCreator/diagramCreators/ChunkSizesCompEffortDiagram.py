...
def createDiagram(inputFileChunkSizes, inputFileCompEffort, dataRowFilter, outputDir, outputFileName, diagramScale, legendScale, legendColumns, isLatex, sort=True, logScale=False, y2LogScale=False):
  ...
  # computational effort per chunk
  legendNames = []
  legendColours = []
  values = []
  dataRowFilters = []
  dataRowFilters.append(dataRowFilter)
  queries, abortedQueries = Utilities.getQueries(inputFileCompEffort, dataRowFilters, isLatex, onlyFinishedQueries=True)
  ...
  with open(inputFileCompEffort, 'rb') as f:
    reader = csv.reader(f, delimiter='\t')
    reader.next()
    for row in reader:
