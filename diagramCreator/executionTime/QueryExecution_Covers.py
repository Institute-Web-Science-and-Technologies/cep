...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    covers[cover][scale][query] = { "Execution Time":(long(row[9])/1000)}
...
for measurementType in ["Execution Time"]:
  ...
  plt.savefig(outputDir+'/queryExecution_'+measurementType+'_sortedByCover.'+imageType, bbox_inches='tight')
  ...
  plt.axis('tight')
  plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=3, mode="expand", borderaxespad=0.)
  plt.savefig(outputDir+'/queryExecution_'+measurementType+'_sortedByNumberOfChunks.'+imageType, bbox_inches='tight')
