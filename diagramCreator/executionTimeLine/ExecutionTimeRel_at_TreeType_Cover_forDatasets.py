    ...
    for baseScale in treeTypes[treeType].keys():
      for baseCover in treeTypes[treeType][baseScale].keys():
        ...
        for i, scale in enumerate(scaleSet):
          ...
          c+=1
        plt.xlabel("Queries")
        plt.ylabel("\\parbox{200pt}{\centering " + measurementType+'\\\\(log scale, change to '+baseCover+"\\\\with "+prettyPrint(baseScale) + ' triples in \\%)'+"}")
        ...
        if imageType=="latex":
          if baseCover == "HASH":
            ...
            fig.tight_layout(rect=(-0.04,-0.045,1.04,0.93))
          latex.savefig(outputDir,'executionTime'+'_relativeTo_'+prettyPrint(baseScale)+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover)
        else:
          plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.4, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/ExecutionTime'+'_relativeTo_'+prettyPrint(baseScale)+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover+'.'+imageType, bbox_inches='tight')
        plt.close('all')
