...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    treeTypes[treeType][scale][cover][query] = { "Execution Time":long(row[12])}
      ...
      for baseCover in treeTypes[treeType][baseScale].keys():
        ...
        for i, scale in enumerate(scaleSet):
          ...
          for query in queryGroups:
           baseDataTransfer = treeTypes[treeType][baseScale][baseCover][query][measurementType]/float(resultSetSize[query][prettyPrint(baseScale)])
           currentDataTransfer = treeTypes[treeType][scale][baseCover][query][measurementType]/float(resultSetSize[query][prettyPrint(scale)])
        ...
        for i, scale in enumerate(scaleSet):
          ...
          newMin = min(dataRows[scale])
          minY=min(minY,newMin)
          c+=1
        plt.xlabel("Queries")
        if imageType=="latex":
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'executionTime'+'_relativeTo_'+prettyPrint(baseScale)+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover+"_perQueryResult")
        else:
          plt.legend(bbox_to_anchor=(0, 1.04, 1, .103), loc=3, ncol=1, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/ExecutionTime'+'_relativeTo_'+prettyPrint(baseScale)+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover+"_perQueryResult"+'.'+imageType, bbox_inches='tight')
        plt.close('all')
