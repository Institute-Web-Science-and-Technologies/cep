    ...
    for baseCover in treeTypes[treeType].keys():
      for baseScale in treeTypes[treeType][baseCover].keys():
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'executionTime'+'_relativeTo_'+baseCover+'_'+measurementType+'_treeType-'+treeType+'_triples-'+prettyPrint(baseScale))
        else:
          plt.legend(bbox_to_anchor=(0, 1.04, 1, .103), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/ExecutionTime'+'_relativeTo_'+baseCover+'_'+measurementType+'_treeType-'+treeType+'_triples-'+prettyPrint(baseScale)+'.'+imageType, bbox_inches='tight')
        plt.close('all')
