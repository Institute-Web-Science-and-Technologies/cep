...
def plotSortedByCover(ax, scale, coverSet, dataRows, queryGroups, measurementType, baseCover):
  ...
  bars = []
  #print baseCover+"-"+scale
  for i, cover in enumerate(coverSet):
    #print "\t"+cover+": "+str(dataRows[cover])
    color=colors[1 if cover=="HASH" else (2 if cover=="HIERARCHICAL" else (3 if cover=="MIN\\_EDGE\\_CUT" else i+1))]
    bars.append(ax.bar(index + i*bar_width+ 0.5*bar_width, np.array(dataRows[cover]), bar_width, color=color, label=cover, linewidth=0.5))
...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    scales[scale][treeType][cover][query] = { "Execution Time":[(long(row[9])),(long(row[10])),(long(row[11]))]}
  ...
  for scale in scales.keys():
    for treeType in scales[scale].keys():
      for baseCover in scales[scale][treeType].keys():
        ...
        plt.figlegend(bars, map(getLabel,bars), loc=3, ncol=3, bbox_to_anchor=(0.005, 0.945, 0.99, 1), mode="expand", borderaxespad=0., borderpad=0.2)
        ...
        if imageType=="latex":
          if len(queryGroups) > 0 and len(midTimeQueries) > 0 and len(longTimeQueries) > 0:
            ...
            fig.subplots_adjust(left=0.28,bottom=0.35,right=1.2,top=0.91)
        #  fig.tight_layout(rect=(0,0,0.9,1))
        if imageType=="latex":
          latex.savefig(outputDir,'plainQueryExecutionTime_executionTime_'+'relativeTo-'+baseCover+'_numberOfChunks-'+scale+'_treeType-'+treeType)
        else:
          plt.savefig(outputDir+'/plainQueryExecutionTime_executionTime_'+'relativeTo-'+baseCover+'_numberOfChunks-'+scale+'_treeType-'+treeType+'.'+imageType, bbox_inches='tight')
        plt.close('all')
