...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    treeTypes[treeType][cover][scale][query] = { "\\# Transferred Packages":long(row[9])}
    ...
    for baseCover in treeTypes[treeType].keys():
      for baseScale in treeTypes[treeType][baseCover].keys():
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'packageTransport'+'_relativeTo_'+baseCover+baseScale+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_sortedByCover')
        else:
          plt.legend(bbox_to_anchor=(-0.2, 1.04, 1.2, .102), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/PackageTransport'+'_relativeTo_'+baseCover+baseScale+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_sortedByCover.'+imageType, bbox_inches='tight')
        plt.close('all')
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'packageTransport'+'_relativeTo_'+baseCover+baseScale+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_sortedByNumberOfChunks')
        else:
          plt.legend(bbox_to_anchor=(-0.4, 1.04, 1.4, .102), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/PackageTransport'+'_relativeTo_'+baseCover+baseScale+'_'+measurementType+'_treeType-'+treeType+'_forAll_covers_sortedByNumberOfChunks.'+imageType, bbox_inches='tight')
        plt.close('all')
