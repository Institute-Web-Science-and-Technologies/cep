    ...
    for baseScale in treeTypes[treeType].keys():
      for baseCover in treeTypes[treeType][baseScale].keys():
        ...
        if imageType=="latex":
          ...
          latex.savefig(outputDir,'packageTransport'+'_relativeTo_'+baseScale+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover)
        else:
          plt.legend(bbox_to_anchor=(0, 1.04, 1, .103), loc=3, ncol=1, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/PackageTransport'+'_relativeTo_'+baseScale+'_'+measurementType+'_treeType-'+treeType+'_cover-'+baseCover+'.'+imageType, bbox_inches='tight')
        plt.close('all')
