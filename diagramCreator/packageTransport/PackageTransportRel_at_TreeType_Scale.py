...
with open(inputFile, 'rb') as f:
  ...
  for row in reader:
    ...
    treeTypes[treeType][cover][scale][query] = { "\\# Transferred Packages":long(row[10])}
  ...
  for treeType in treeTypes.keys():
    ...
    for baseCover in treeTypes[treeType].keys():
      ...
      for baseScale in treeTypes[treeType][baseCover].keys():
        ...
        if imageType=="latex":
          plt.legend(bbox_to_anchor=(-0.9, 1.07, 1.9, .102), loc=3, ncol=1, mode="expand", borderaxespad=0., borderpad=0.2, labelspacing=0.2)
          fig.tight_layout(rect=(-0.06,-0.045,1.07,0.92))
          latex.savefig(outputDir,'packageTransport'+'_relativeTo_'+baseCover+'_'+measurementType+'_treeType-'+treeType+'_scale-'+baseScale)
        else:
          plt.legend(bbox_to_anchor=(-0.45, 1.04, 1.5, .105), loc=3, ncol=2, mode="expand", borderaxespad=0.)
          plt.savefig(outputDir+'/PackageTransport'+'_relativeTo_'+baseCover+'_'+measurementType+'_treeType-'+treeType+'_scale-'+baseScale+'.'+imageType, bbox_inches='tight')
        plt.close('all')
